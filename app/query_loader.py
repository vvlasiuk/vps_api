...
# Патерни пакетного режиму (мають збігатися з логікою onec_router.py):
#   • _RESULT_TT_PUT_RE — те, що роутер розпізнає як пакетний запит
#     (готує ПОМЕСТИТЬ ТТ_РЕЗУЛЬТАТЗАПИТУ_ → фінальний пакет ИЗ цієї ТТ);
#   • _RESULT_TT_ANY_RE — будь-яка згадка службового імені (для старт-чеку).
# Якщо .sel згадує ТТ_РЕЗУЛЬТАТЗАПИТУ_, але БЕЗ ПОМЕСТИТЬ — роутер піде
# звичайною гілкою ИЗ(...) КАК Вложенный, і батч зламається. Ловимо це при старті.
_RESULT_TT_PUT_RE = re.compile(r"ПОМЕСТИТЬ\s+ТТ_РЕЗУЛЬТАТЗАПИТУ_", re.IGNORECASE)
_RESULT_TT_ANY_RE = re.compile(r"ТТ_РЕЗУЛЬТАТЗАПИТУ_", re.IGNORECASE)
...
def load_queries() -> dict:
    ...
    for root, dirs, files in os.walk(QUERIES_DIR):
        for filename in files:
            ...
            # Старт-чек пакетного режиму: згадка ТТ_РЕЗУЛЬТАТЗАПИТУ_ без ПОМЕСТИТЬ.
            # Роутер такий запит НЕ вважає пакетним і піде звичайною гілкою → батч
            # зламається в рантаймі. Попереджаємо при старті (не блокуємо завантаження).
            if _RESULT_TT_ANY_RE.search(query_text) and not _RESULT_TT_PUT_RE.search(query_text):
                print(f"[query_loader] УВАГА '{file_base}': згадано ТТ_РЕЗУЛЬТАТЗАПИТУ_, "
                      f"але немає 'ПОМЕСТИТЬ ТТ_РЕЗУЛЬТАТЗАПИТУ_'. "
                      f"Роутер виконає запит у звичайному режимі — пакет із temp-таблицями "
                      f"зламається. Заверши підготовку на ПОМЕСТИТЬ ТТ_РЕЗУЛЬТАТЗАПИТУ_.")
