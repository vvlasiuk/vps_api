...
`{"kind":"ref","object":"Перечисление.X"}`), АЛЕ на відміну від Довідник/Документ-
посилань:

- **Читання** (query): 1С HS-конвертер (`VPS.Module.bsl`, `КонвертуватиЗначення`)
  повертає для перелічень обʼєкт `{type, presentation, value}` — ідентифікатор
  лежить у ключі **"value"** (XML-рядок значення, `XMLСтрока(Значення)`), а НЕ в
  "guid", як для звичайних посилань.
- Тому в `.json` запиту тип такого поля (`fields[].type`) має лишатись **"ref"**,
  а НЕ "string" — якщо поставити "string", бекенд стриже обʼєкт до самого
  `presentation`, і сирий `.value` (потрібний для запису назад) губиться
  безповоротно.
- **Запис** (save_cat/save_doc): `VPS.КонвертуватиПараметр` для `type:"ref"`
  викликає `КонвертуватиПосилання(value.type, value.guid) → XMLValue(Тип, рядок)`.
  Для перелічень у payload треба слати:
  `{type:"ref", value:{type:"Перечисление.ІмяПерелічення", guid: <.value з читання>}}`
  — ключ `"guid"` лишається тим самим навіть для перелічення (це просто назва
  параметра функції, а не вимога справжнього UUID; XMLValue приймає туди будь-
  який коректний XML-рядок значення).
- На фронті: показ — через `.presentation` (як і для звичайних ref, `_pres()`
  працює однаково); запис — через `.value`, НІКОЛИ через `.guid` (якого в обʼєкта
  перелічення просто немає) і НІКОЛИ через `.presentation` (це людський текст,
  не XML-ідентифікатор).
